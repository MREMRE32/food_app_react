...
const DBNewItem = () => {
  ...
  const [imageDowloadURL, setImageDownloadURL] = useState(null);

  const alert = useSelector((state) => state.alert);
  const dispatch = useDispatch();

  {
    /*Upload Files using firebase */
  }
  const uploadImage = (e) => {
    setIsLoading(true);
  ...
  const deleteImageFromFirebase = () => {
    setIsLoading(true);
    const deleteRef = ref(storage, imageDowloadURL);
    ...
    <div className="flex items-center justify-center flex-col pt-6 px-24 w-full">
      <div className="border border-gray-300 rounded-md p-4 w-full flex flex-col items-center justify-center gap-4">
          ...
          {statuses &&
            statuses?.map((data) => (
              <p
                ...
                className={`px-4 py-3 rounded-md text-xl text-textColor font-semibold cursor-pointer hover:shadow-md border bordergray-200 backdrop-blur-md ${
        ...
        <InputValueField
          ...
          stateValue={price}
        />
        {/*can add spinner https://flowbite.com/docs/components/spinner/ */}
                ...
                <>
                  <div className="relative w-full h-full overflow-hidden rounded-md">
                    ...
                    <motion.button
                      ...
                      onClick={() => deleteImageFromFirebase(imageDowloadURL)}
                    >
